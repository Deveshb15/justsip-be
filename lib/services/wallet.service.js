...
const resolveAssetId = (token) => {
    // If token is a valid Coinbase asset, use it
...
class WalletService {
    ...
    async trade(wallet_id, amount, from_token, to_token) {
        if (!wallet_id || !amount || !from_token || !to_token) {
            throw new Error('wallet_id, amount, from_token, and to_token are required');
        }
    ...
    async createSIP(wallet_id, from_token, to_token, amount, frequency) {
        if (!wallet_id || !from_token || !to_token || !amount || !frequency) {
            throw new Error('wallet_id, from_token, to_token, amount, and frequency are required');
        }
        ...
        try {
            ...
            const { data: sipData, error: sipError } = await supabase
                ...
                .insert([{
                    ...
                    status: isInsufficientFundsError(error) ? SIP_STATUS.INSUFFICIENT_FUNDS : SIP_STATUS.PAUSED,
                    total_executions: 0
                }])
                .select();
    ...
    async executeSIP(sip_id) {
        ...
        try {
            ...
            // Update SIP status based on error
            const newStatus = isInsufficientFundsError(error) 
            ...
            throw {
                message: isInsufficientFundsError(error)
                    ? 'Insufficient funds for SIP execution'
                    : 'Failed to execute SIP trade',
                sip_id,
                status: newStatus
            };
        }
    ...
    async updateSIPStatus(sip_id, wallet_id, status) {
        ...
        if (!Object.values(SIP_STATUS).includes(status)) {
            throw new Error(`Invalid status. Must be one of: ${Object.values(SIP_STATUS).join(', ')}`);
        }
