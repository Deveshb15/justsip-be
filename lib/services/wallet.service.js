...
class WalletService {
  ...
  async createWallet(name, wallet_address, image_url) {
    ...
    console.log("EXISTING USER", existingUser , searchError)
  ...
  async createSIP(wallet_id, from_token, to_token, amount, frequency) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        console.error(
          `Error executing initial trade (attempt ${attempt}/${MAX_RETRIES}):`,
          error
        );
        lastError = error;
    ...
    // If all retries failed, create SIP with appropriate status
    console.error("All retry attempts failed for initial trade");
      ...
      .insert([
        {
          ...
          status: isInsufficientFundsError(lastError)
    ...
    return {
      sip: sipData[0],
      error: isInsufficientFundsError(lastError)
